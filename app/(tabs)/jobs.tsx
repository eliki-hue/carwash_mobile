...
export default function JobsScreen() {
  ...
  // Load jobs when tab changes and reference data is ready
  useEffect(() => {
    if (dataLoaded) {
      loadJobs();
    }
  ...
  const loadJobs = async () => {
    try {
      console.log('Loading jobs for status:', activeTab);
      const response = await api.get(`/jobs/?status=${activeTab}`);
  ...
  const getServiceName = (serviceId: number) => {
    ...
    if (!service) {
      console.log(`Service not found for ID: ${serviceId}, available services:`, services.map(s => ({ id: s.id, name: s.name })));
  ...
  const getVehicleName = (vehicleId: number) => {
    ...
    if (!vehicle) {
      console.log(`Vehicle not found for ID: ${vehicleId}, available vehicles:`, vehicles.map(v => ({ id: v.id, name: v.name })));
  ...
  const renderJobCard = ({ item }: { item: Job }) => {
    ...
    const vehicleName = getVehicleName(item.vehicle_type);
  ...
  if (loading) {
    return <LoadingSpinner message="Loading services and vehicles..." />;
  }
